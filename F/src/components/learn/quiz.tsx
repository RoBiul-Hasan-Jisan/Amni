...
import { useProgress } from "@/components/learn/progress-tracker";
...
interface QuizProps {
  ...
  title?: string;
  /** When provided, the final score is saved to the learner's progress. */
  topicId?: string;
  subtopicSlug?: string;
}

export function Quiz({ questions, title = "Quick Quiz", topicId, subtopicSlug }: QuizProps) {
  ...
  React.useEffect(() => {
    if (showResult && topicId && subtopicSlug) {
      saveQuizScore(topicId, subtopicSlug, Math.round((score / questions.length) * 100));
    }
